...
def GenerateMailMessages(userName, password, resumeFile):
    ...
    for msgId in data[0].split()[-1:-2:-1]:
        typ, response = imapSession.fetch(msgId, '(X-GM-LABELS)')
        print(response)
        if 'Downloaded' in response[0]:
            NewMsgIDs.add(msgId)
            ...
            if msgId not in ProcessedMsgIDs:
                yield email.message_from_string(emailBody) #yield returns a generator
                ProcessedMsgIDs.add(msgId)
                with open(resumeFile, "a") as resume:
                    resume.write('{id},'.format(id=msgId))
                typ, response = imapSession.store(msgId, '+X-GM-LABELS', r'(Downloaded)')
    imapSession.close()
    imapSession.logout()
